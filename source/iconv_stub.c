...
static void get_substitute(struct mliconv_t *internal, char const **substitute,
...
static int convert_one_sequence(char const *tocode, char const *fromcode,
	char in, char **outbuf, size_t *outbytesleft)
{
	int result;
	iconv_t handle = iconv_open(tocode, fromcode);
	if(handle == (iconv_t)-1){
...
static int put_substitute(struct mliconv_t *internal, char **outbuf,
	size_t *outbytesleft)
{
...
static size_t get_min_sequence_in_fromcode(struct mliconv_t *internal)
{
	...
	if(min_sequence_in_fromcode < 0){
		...
		char *d = outbuffer;
		size_t d_len = MAX_SEQUENCE;
		if(convert_one_sequence(internal->fromcode, latin1,
...
static void skip_min_sequence(struct mliconv_t *internal, char **inbuf,
	size_t *inbytesleft)
{
